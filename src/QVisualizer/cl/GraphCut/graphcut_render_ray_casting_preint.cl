...
constant sampler_t integrationTableSampler = CLK_NORMALIZED_COORDS_TRUE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_LINEAR;
    ...
    )
{
...
#ifdef cl_image_2d
    float4 sampleValue = read_imagef(volumeTexture, volumeSampler, samplePosition.xy);
#else
    float4 sampleValue = read_imagef(volumeTexture, volumeSampler, samplePosition);
#endif
    float2 sampleScalar = (float2)(clamp(sampleValue.w, windowMin, windowMax));
    ...
    for(int i = 0; i < sampleNumber; i++)
    {
        ...
        // accumulate result
        sampleScalar.y = sampleScalar.x;
        sampleScalar.x = clamp(sampleValue.w, windowMin, windowMax);
...
#endif
        const float4 color = read_imagef(integrationTable, integrationTableSampler, sampleScalar);
        if (*cut > 0.0f) pixelColor += color * sampleStepScale * volumeStepSize * exp(-pixelColor.w);
    ...
    )
{
    ...
    for(int i = 0; i < sampleNumber; i++)
    {
        ...
        const float4 color = read_imagef(integrationTable, integrationTableSampler, sampleScalar);
        pixelColor += color * sampleStepScale * volumeStepSize * exp(-pixelColor.w);
