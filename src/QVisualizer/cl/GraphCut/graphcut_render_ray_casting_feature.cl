    ...
    )
{
    ...
    for(int i = 0; i < sampleNumber; i++)
    {
        ...
        if (*cut > 0.0f) pixelColor += (float4)(sampleValue.xyz, 1.0) * color.w * sampleStepScale * volumeStepSize * exp(-pixelColor.w);
    ...
    )
{
    ...
    for(int i = 0; i < sampleNumber; i++)
    {
        ...
        pixelColor += (float4)(sampleValue.xyz, 1.0) * color.w * sampleStepScale * volumeStepSize * exp(-pixelColor.w);
